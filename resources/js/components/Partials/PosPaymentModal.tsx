...
import { useForm, router } from '@inertiajs/react'; // Keep useForm as per your code
...
// --- TYPES (Updated for POS Inputs) ---
interface PosPaymentModalProps {
    isOpen: boolean;
    onClose: () => void;
    totalAmount: number;     // 🟢 From POS
    customer: any;           // 🟢 From POS
    cartData: any[];         // 🟢 From POS
}
...
// --- PAYMENT ROW (Exact Copy - Includes C2B/Manual) ---
const PaymentRow = ({
    line,
    onChange,
    onRemove,
    ...
    // --- B. MANUAL VERIFY LOGIC ---
    const handleManualVerify = async () => {
        if (!line.mpesaPhone || line.mpesaPhone.length < 9) {
            alert("Please enter phone number to verify.");
            return;
        }
        if (!line.amount || Number(line.amount) <= 0) {
        ...
        try {
            ...
            setStkStatus('failed');
            setStkError(error.response?.data?.error || "Payment not found yet.");
        }
    };
    ...
    return (
        <div className={`space-y-4 p-4 rounded-xl border transition-all ${containerClass}`}>
            ...
            {/* M-Pesa Content (INCLUDES C2B/MANUAL AS REQUESTED) */}
            {line.method === 'mpesa' && (
...
// --- 🟢 MAIN POS MODAL (The one that submits to POS Controller) ---
export default function PosPaymentModal({ isOpen, onClose, totalAmount, customer, cartData }: PosPaymentModalProps) {
    const [mode, setMode] = useState<'single' | 'split'>('single');
    const [localProcessing, setLocalProcessing] = useState(false);

    // 🟢 POS Adaptation: Calculate inputs derived from props
    const creditLimit = customer ? Number(customer.available_credit ?? customer.credit_limit ?? 0) : 0;
    ...
    const isSubmitDisabled = localProcessing || processing || lines.some(l =>
        (l.method === 'mpesa' || l.method === 'credit_limit') && !l.verified
    );

    // 🟢 POS SUBMIT LOGIC (Updated to match POS requirements)
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (isSubmitDisabled) return;
        setLocalProcessing(true);

        const finalPayments = lines.map(l => ({
            amount: Number(l.amount),
            method: l.method,
            transaction_ref: l.transaction_ref,
            mpesaMode: l.method === 'mpesa' ? l.mpesaMode : null,
            mpesaPhone: l.method === 'mpesa' ? l.mpesaPhone : null,
            payment_date: new Date().toISOString().split('T')[0]
        }));

        // 🟢 Submits to POS.STORE with Cart Data + Preserve State
        router.post(route('pos.store'), {
            ...
            preserveState: true, // 🟢 CRITICAL: Keeps page alive for receipt modal
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleAutoSubmit = () => {
        console.log("Verification successful. Waiting for confirmation.");
    };
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
                <DialogHeader className="mb-2">
                    ...
                    <DialogDescription>
                        Customer: <span className="font-bold text-gray-800">{customer?.name || 'Walk-In'}</span>
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid grid-cols-3 gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200 mb-6">
                    ...
                    <div>
                        <div className="text-xs font-semibold text-gray-500 uppercase">Balance / Change</div>
