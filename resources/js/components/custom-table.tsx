...
// ✅ Normalizes stored image paths into a browser-loadable URL
// Accepts:
// - "products/main/x.jpg"
...
export const CustomTable: React.FC<CustomTableProps> = ({
    ...
    return (
      <div className="flex justify-center items-center gap-1">
        {mainActions.map((action, index) => {
          ...
          return (
            <Button key={index} size="sm" className={action.className} onClick={handleClick}>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {moreActions.map((action, index) => {
                ...
                return (
                  <DropdownMenuItem key={index} onClick={handleClick} className="flex items-center gap-2 cursor-pointer">
  ...
  const handleApplyDateFilter = () => {
    if (onDateFilterChange) {
      onDateFilterChange(dateFrom || null, dateTo || null);
    }
  };
            ...
            {data.length > 0 ? (
              data.map((row, index) => {
                ...
                return (
                  <tr
                    key={row.id}
                    className={`transition-colors ${isSelected ? 'bg-orange-50' : 'hover:bg-gray-50'}`}
                  >
                    ...
                    {columns.map((col) => {
                      ...
                      return (
                        <td key={col.key} className={`border px-4 py-2 text-center text-gray-700 ${col.className || ''}`}>
                          {col.isImage ? (
                            <div className="flex justify-center">
                              {row[col.key] ? (
                                <img
                                  src={toStorageUrl(String(row[col.key]))}
                                  alt="Image"
                            ...
                            <span className="font-mono">KSH {Number(row[col.key] ?? 0).toFixed(2)}</span>
                          ) : col.type === 'boolean' ? (
                            <div className="flex justify-center">
                              {row[col.key] ? (
                            ...
                            row[col.key] ?? <span className="text-gray-400 italic">—</span>
                          )}
                        </td>
                      );
                    })}
              ...
              <tr>
                <td colSpan={columns.length + 2} className="text-md py-6 text-center font-semibold text-orange-500">
                  No data available
                </td>
              </tr>
            )}
        ...
        <div className="p-3 border-t bg-orange-50 text-sm text-orange-700 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                {onBulkExportPDF && <DropdownMenuItem onClick={handleBulkExportPDFClick}>Export selected (PDF)</DropdownMenuItem>}
                {onBulkExportExcel && <DropdownMenuItem onClick={handleBulkExportExcelClick}>Export selected (Excel)</DropdownMenuItem>}
                {onBulkDelete && <DropdownMenuItem onClick={handleBulkDeleteClick}>Delete selected</DropdownMenuItem>}
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
