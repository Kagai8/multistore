...
interface FieldProps {
  ...
  hidden?: boolean | ((mode: 'create' | 'view' | 'edit', data?: Record<string, any>) => boolean);
}
...
interface FinanceModalFormProps {
  title: string;
  description?: string;
  fields: FieldGroup[];
  buttons: ButtonProps[];
  ...
  // 🟢 NEW PROP: To control where children (Item Editor) appear
...
export const FinanceModalForm: React.FC<FinanceModalFormProps> = ({
  addButton,
  title,
  description,
  fields,
  ...
  children,
  currentUserContext,
  childrenPosition, // 🟢 Destructure new prop
}) => {
  ...
  const renderField = (field: FieldProps) => {
    ...
    return (
      <div key={field.key} className={`grid gap-2 ${colClass}`}>
        <Label htmlFor={field.id} className={isDisabled ? 'opacity-70' : ''}>
            {field.label}
        </Label>
        ...
        {field.type === 'textarea' ? (
          ...
          <div className="flex items-center space-x-2 pt-2">
            <Checkbox
              ...
              disabled={isDisabled}
            />
            <Label htmlFor={field.id} className="text-sm font-normal text-gray-500 cursor-pointer">
            ...
            >
                {(optionsMap[field.optionsSource as string] ?? []).map((option) => (
                    <div key={option.key} className="flex items-center space-x-2">
                        ...
                        <Label htmlFor={`${field.id}-${option.value}`} className="font-normal cursor-pointer text-sm">
                            {option.label}
                        </Label>
                    </div>
                ))}
                ...
                {(multiImagePreviews || []).map((src, index) => {
                    ...
                    return (
                        <div key={`old-img-${index}`} className="relative h-20 w-20">
                            <img src={finalSrc} alt={`Image ${index + 1}`} className="h-full w-full rounded object-cover border" />
                            {mode !== 'view' && (
                                <button type="button" onClick={() => handleDeleteMultiImage(src)} className="absolute top-0 right-0 p-1 bg-red-600 rounded-full text-white text-xs hover:bg-red-700 transition-colors" title="Remove image">❌</button>
                            )}
                        </div>
                    );
                })}
                {Array.isArray(data[field.name]) && data[field.name].map((file: File, index: number) => {
                    if (file instanceof File) return <img key={`new-img-${index}`} src={URL.createObjectURL(file)} alt={`New Image ${index + 1}`} className="h-20 w-20 rounded object-cover border border-dashed border-green-500" />;
                    return null;
                })}
              </div>
            )}
  ...
  return (
    ...
    <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
        ...
        {/* 🟢 NEW LOGIC: Iterate groups and inject children if position matches */}
        {(fields ?? []).map((group, groupIndex) => (
            <React.Fragment key={groupIndex}>
                ...
                {/* 🟢 Inject Children (Items Editor) HERE if index matches */}
                {childrenPosition === groupIndex && (
                    <div className="my-4">
                        {children}
                    </div>
                )}
        ...
        {/* 🟢 Fallback: If position not specified, render at bottom (Legacy support) */}
        {childrenPosition === undefined && children}
        ...
        <DialogFooter className="sticky bottom-0 bg-white dark:bg-gray-900 border-t pt-4 z-10">
           {buttons.map((btn) => {
            ...
            if (mode !== 'view') {
                return (
                <Button key={btn.key} type={btn.type as any} variant={btn.variant as any} className={btn.className} disabled={processing}>
                    {btn.label}
                </Button>
                );
            }
...
export default FinanceModalForm;
